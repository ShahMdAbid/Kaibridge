...
| **DRC: `[unconnected_items]`** | Unrouted airwires remaining on board | Check if net was unrouted or pads are too cramped for routing channels. Adjust component placement spacing using `kaibridge_apply_ops_layout` or run `kaibridge_auto_relax_layout`, then re-run `kaibridge_route_pcb`. |
...
| **Freerouting: Timeout expired** | Dense placement with blocked routing channels causing endless rip-up loops | Increase component spacing using `kaibridge_auto_relax_layout`, or widen board outline by 10–20% using `{"op": "board.set_size", ...}`, then re-run `kaibridge_route_pcb`. |
...
| **`ModuleNotFoundError: No module named 'pcbnew'`** | Running script from standard Python interpreter rather than KiCad's bundled Python | Ensure backend tools call `load_kicad_python()` to dispatch to KiCad's bundled Python 3.11 environment (`C:\Program Files\KiCad\10.0\bin\python.exe`). |
