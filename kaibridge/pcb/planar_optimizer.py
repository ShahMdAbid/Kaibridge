"""
kaibridge/pcb/planar_optimizer.py -- Simulated Annealing Planar Layout Optimizer.

Minimizes cross-net airwire crossings using Kruskal's MST, strict 2D segment intersections,
and component proximity potentials.
"""
...
from ..core.geometry import ccw, rotate_point, segments_intersect
...
'''
    res = subprocess.run([str(kicad_py), "-c", script], capture_output=True, text=True, check=True)
    out = res.stdout
...
class PlanarLayoutOptimizer:
    def __init__(
        ...
        pcb_files = list(self.proj_dir.glob("*.kicad_pcb"))
        if not pcb_files:
            raise FileNotFoundError(f"No .kicad_pcb found in {self.proj_dir}")
        self.pcb_path = pcb_files[0]
...
def optimize_planar_layout(
    project_path: str | Path,
    steps: int = 1000,
    initial_temp: float = 70.0,
    commit: bool = True
) -> Dict[str, Any]:
    """Programmatic API for simulated annealing planar placement and floorplanning."""
    ...
    if not has_places:
        try:
            from kaibridge.pcb.hierarchical_floorplan import UniversalHierarchicalFloorplanner
            planner = UniversalHierarchicalFloorplanner(
                fp_data=opt.fp_data,
                design=opt.design,
                nets=opt.nets,
                board_width=bw,
    ...
    best_ops, crossings = opt.optimize(init_ops, steps=steps, temp_init=initial_temp)
    ...
    return {
        "success": True,
        "crossings": crossings,
        "ops_file": str(out_ops_file),
        "committed": committed,
        "ops_count": len(best_ops)
    }
...
def main(argv=None):
    ...
    if res.get("success"):
        print(f"\nSaved optimized ops to: {res.get('ops_file')}")
