"""Kaibridge Project & Library Bootstrap Engine (kaibridge.core.init).
Creates / upserts:
    ...
    <PROJECT_DIR>/kaibridge_dump/warning.md
"""
...
def format_version(path: Path) -> str:
...
def detect_versions(cfg: Dict[str, Any]):
    """(lib-table version, .kicad_sym version) taken from this machine's KiCad."""
    table_src = cfg.get("kicad_config_dir")
    if table_src:
        table_src = table_src / "sym-lib-table"
    if not table_src or not table_src.is_file():
        return "7", "20231120"
    symbol_dir = cfg.get("kicad_symbol_dir")
    stock = sorted(symbol_dir.glob("*.kicad_sym")) if symbol_dir else []
    sym_v = format_version(stock[0]) if stock else "20231120"
    return format_version(table_src), sym_v


def lib_entry(name: str, uri_tpl: str) -> str:
    return (
...
def listed_names(text: str, file_name: str) -> list[str]:
...
def upsert_table(file: Path, tag: str, version: str, name: str, uri_tpl: str) -> str:
    if not file.exists():
        file.write_text(
...
def init_libraries(project_dir: str | Path, name: str = "kaibridge", layers: int = 2) -> Dict[str, Any]:
    name = name.strip()
