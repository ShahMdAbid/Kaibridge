---
name: Kaibridge Stacked Pins (Overlapping Labels) Fix
description: How to avoid overlapping, stacked global net labels when designing JSON files for the Kaibridge KiCad schematic generator.
---

# Handling Stacked Pins in Kaibridge `design.json`

When defining nets in a `design.json` file for the `Kaibridge` plugin, **do not list multiple pins for a component if those pins are physically stacked (located at the exact same coordinates) in the KiCad symbol.**
