...
<aside>
🎯
**Doctrine, not suggestions.** This is the canonical, production-grade placement law for AI agents and human designers translating a schematic into a manufacturable, electrically robust PCB layout in **abIDE**. It is project-agnostic and vision-loop native: every rule states a *target*, a *tolerance*, and the *failure it prevents*, so it can be checked by geometry math, by a rendered snapshot, or by a human in one glance.
</aside>

**Version:** 2.0  
**Scope:** Every abIDE-driven board, any project.  
...
* `DFM-01` **Collision Awareness:** While the `Geometry Gate` automatically resolves micro-collisions, the AI MUST calculate nominal coordinates with enough breathing room so the geometry collision solver doesn't heavily distort the layout.
...
## 🤖 5. The abIDE Execution Protocol
...
4. **The Geometry Gate:** The built-in solver automatically detects micro-collisions and performs deterministic courtyard separation (`geometry.separate()`) across up to 80 passes to snap components into 100% legal, zero-overlap positions.
5. **The Vision Gate:** The AI verifies the layout by analyzing the SVG generated by `pcb_snapshot.py`, citing rule IDs (e.g., `DEC-02`, `THM-02`) in its structured feedback table if corrections are needed.
