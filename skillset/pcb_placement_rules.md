...
* `DFM-01` **Collision Awareness:** While the `Geometry Gate` automatically resolves micro-collisions, the AI MUST calculate nominal coordinates with enough breathing room so the physics relaxer doesn't heavily distort the layout.
...
4. **The Geometry Gate:** The built-in solver automatically detects micro-collisions and performs physics relaxation (`geometry.separate()`) across up to 80 passes to snap components into 100% legal, zero-overlap positions.
